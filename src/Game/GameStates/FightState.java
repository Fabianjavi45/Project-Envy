...
public class FightState extends InWorldState{
	...
	private int entityY;
	public static String LastEnemy="null";
	...
	public FightState(Handler handler ,BaseHostileEntity enemy, String prevState) {
		...
		*/
		...
		if(enemy.type.equals("Boss")) {
			handler.getGame().getMusicHandler().set_changeMusic("res/music/DialgaTheme.mp3");
			handler.getGame().getMusicHandler().play();
			handler.getGame().getMusicHandler().setVolume(0.2);
		}
		else if(enemy.type.equals("sBoss")) {
			handler.getGame().getMusicHandler().set_changeMusic("res/music/DialgaTheme.mp3");
			handler.getGame().getMusicHandler().play();
			handler.getGame().getMusicHandler().setVolume(0.2);
		}
	...
	private void drawEntities(Graphics2D g2) {
		...
		if(enemy.type.equals("Boss")) {
			g2.drawImage(enemy.getIdle(), enemyRect.x-200, enemyRect.y-200, enemyRect.width+200, enemyRect.height+200, null);
		}
		else if(enemy.type.equals("sBoss")) {
			g2.drawImage(enemy.getIdle(), enemyRect.x-400, enemyRect.y-400, enemyRect.width+400, enemyRect.height+400, null);
		}
		else g2.drawImage(enemy.getIdle(), enemyRect.x, enemyRect.y, enemyRect.width, enemyRect.height, null);
	}
	...
	private void callSkill(Graphics g) {
		if(handler.getEntityManager().getPlayer().getSkill().equals("None")) {
			//g.drawImage(Images.SkillNone,0,0,handler.getWidth()/2,handler.getHeight()/2, null);
			endTurn=true;
			attacked=false;
			skill=false;
			endTurn=false;
			...
			if(enemy.name.equals("Primal Minion")) {
				g.drawImage(Images.tint(enemy.getIdle(),0,0,Math.max(0, blue--)),enemyRect.x-200,enemyRect.y-200,enemyRect.width+200,enemyRect.height+200,null);
			}
			else g.drawImage(Images.tint(enemy.getIdle(),0,0,Math.max(0, blue--)),enemyRect.x,enemyRect.y,enemyRect.width,enemyRect.height,null);
	...
	private void Eattack(Graphics g) {
		...
		if(!playerAttackMode.end){
			playerAttackMode.tick();
			if(enemy.name.equals("Primal Minion")) {
				g.drawImage(Images.tint(playerAttackMode.getCurrentFrame(),0,0,2),enemyRect.x-215,enemyRect.y-205,enemyRect.width+210,enemyRect.height+210,null);
			}
			else if(enemy.name.equals("Castle Monster")) {
				g.drawImage(Images.tint(playerAttackMode.getCurrentFrame(),0,0,2),enemyRect.x-415,enemyRect.y-405,enemyRect.width+410,enemyRect.height+410,null);
			}
			else g.drawImage(Images.tint(playerAttackMode.getCurrentFrame(),0,0,2),enemyRect.x-15,enemyRect.y-5,enemyRect.width+10,enemyRect.height+10,null);
		}
		else {
	...
	private void Edefend(Graphics g) {
		...
		playerDefenceMode.tick();
		if(enemy.name.equals("Primal Minion")) {
			g.drawImage(Images.tint(playerDefenceMode.getCurrentFrame(),0,0,2),enemyRect.x-215,enemyRect.y-205,enemyRect.width+210,enemyRect.height+210,null);
		}
		else if(enemy.name.equals("Castle Monster")) {
			g.drawImage(Images.tint(playerDefenceMode.getCurrentFrame(),0,0,2),enemyRect.x-415,enemyRect.y-405,enemyRect.width+410,enemyRect.height+410,null);
		}
		else g.drawImage(Images.tint(playerDefenceMode.getCurrentFrame(),0,0,2),enemyRect.x-15,enemyRect.y-5,enemyRect.width+10,enemyRect.height+10,null);
