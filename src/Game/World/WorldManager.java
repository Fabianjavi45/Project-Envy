...
public class WorldManager {
	...
	public WorldManager(Handler handler, EntityManager entityManager) {
		...
		D= new Dusknoir(handler, 1640, 50);
		this.entityManager.AddEntity(D);
		
		
		this.entityManager.AddEntity(handler.newEnemy(Images.PEnemyIdle,handler,500, 800,"MapState","Dark Minion","None","EnemyOne",100,25,40,1,8,12,20,10,20,10,1,5,"None","Fire",null,null)); // lvl 0 dificulty
		this.entityManager.AddEntity(handler.newEnemy(Images.PEnemyIdle,handler,1400, 600,"MapState","Dark Minion","None","EnemyOne",100,25,40,1,8,12,20,10,20,10,1,5,"None","Fire",null,null)); // lvl 0 dificulty
		this.entityManager.AddEntity(handler.newEnemy(Images.PEnemyIdle,handler,2400, -200,"MapState","Dark Minion","None","EnemyOne",100,25,40,1,8,12,20,10,20,10,1,5,"None","Fire",null,null)); // lvl 0 dificulty
		this.entityManager.AddEntity(handler.newEnemy(Images.QEnemyIdle,handler,1200, -50,"MapState","Primal Minion","None","Boss",100,25,40,25,8,12,20,10,20,10,1,5,"None","Fire",null,null));
	...
	public void tick() {
		if(Player.pNear) {
			this.entityManager.RemoveEntity(D);
			this.entityManager.AddEntity(new Dusknoir(handler, 1715, 50));
			worldWalls.add(new Walls(handler, 1715, 50, 105, 105, "Wall"));
			Player.QuestComplete=false;
			Player.pNear=false;
		}
	...
	private void AddWalls() {
		...
		//Town Square
		worldWalls.add(new Walls(handler, 1090,400, 200, 270, "Door Town"));
		
		//Second Island
		worldWalls.add(new Walls(handler, 3107,3148, 56, 184, "Wall"));
		worldWalls.add(new Walls(handler, 2553, 2368, 98, 40, "Door Castle"));
