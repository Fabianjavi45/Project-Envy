...
public class SBoss extends BaseHostileEntity implements Fighter{

    Rectangle sBoss;
    int width, height;

    public SBoss(Handler handler, int xPosition, int yPosition, String state, String name, String area, BufferedImage[] animFrames) {
    ...
    public void render(Graphics g) {
        ...
        if(handler.getArea().equals(this.Area)) {
        	...
        	sBoss = new Rectangle((int) (handler.getXInWorldDisplacement() + getXOffset()),
                    (int) (handler.getYInWorldDisplacement() + getYOffset()), 460, 460);
        	}
            g2.setColor(Color.black);
            g.drawImage(Images.Ray,sBoss.x,sBoss.y,sBoss.width,sBoss.height,null);
            
            if (sBoss.intersects(handler.getEntityManager().getPlayer().getCollision())) {
